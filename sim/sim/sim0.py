...
class Sim0_Node(rclpy.node.Node):
    def __init__(self):
        ...
        # delays when we're starting up.
        self.robot_description_publisher = self.create_publisher(std_msgs.msg.String, 'robot_description', qos)
        self.robot_description_timer = self.create_timer(5.0, self.publish_robot_description)
        self.publish_robot_description()

        # Get ready to publish a marker array showing the robot's path.
        self.past_locations = [ self.pose[0:2].copy() ]
        self.past_locations_publisher = self.create_publisher(visualization_msgs.msg.MarkerArray, '/past_locations', 1)
        self.past_locations_timer = self.create_timer(0.2, self.publish_past_locations)
    ...
    def update_pose(self):
        ...
        self.pose[0] += math.cos(self.pose[2]) * self.twist.linear.x
        self.pose[1] += math.sin(self.pose[2]) * self.twist.linear.x
    ...
    def publish_past_locations(self):
        ...
        path_color = std_msgs.msg.ColorRGBA(r=1.0, g=0.2, b=0.2, a=1.0)
        for location in self.past_locations:
        ...
        self.past_locations_publisher.publish(markers)
